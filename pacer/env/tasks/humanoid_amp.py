...
class HumanoidAMP(Humanoid):
    ...
    def _reset_actors(self, env_ids):
        if (self._state_init == HumanoidAMP.StateInit.Default):
            ...
            self._state_init = HumanoidAMP.StateInit["Keep"]#这里的逻辑非常不合理  先改成这样  后面再说吧
        elif (self._state_init == HumanoidAMP.StateInit.Hybrid):
            self._reset_hybrid_state_init(env_ids)
        elif (self._state_init == HumanoidAMP.StateInit.Keep):
            return
        else:
            assert (
                False
    ...
    def _init_amp_obs(self, env_ids):
        ...
        if (len(self._reset_ref_env_ids) > 0):
            self._init_amp_obs_ref(self._reset_ref_env_ids,#self._hist_amp_obs_buf
    ...
    def _update_camera(self):
        ...
        # new_cam_pos = gymapi.Vec3(20,0,17)
        # new_cam_target = gymapi.Vec3(0,0,0)
        x,y,z = self.cam_pos
