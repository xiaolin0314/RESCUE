...
class MotionDiffusion(nn.Module):
    def __init__(self, input_feats, nstyles, njoints, nfeats,nembeds, rot_req, clip_len,
        ...
        self.source_motion_process = MotionProcess(self.input_feats, self.latent_dim)
        self.style_motion_process = MotionProcess(self.input_feats, self.latent_dim)
        ...
        self.output_process = OutputProcess(self.input_feats, self.latent_dim, self.njoints, self.nfeats,self.nembeds)


    def forward(self, x, timesteps, source_pos, style_idx):
        ...
        source_pos_emb = self.source_motion_process(source_pos)  # 
        
        future_motion_emb = self.style_motion_process(x) 
        
        xseq = torch.cat((time_emb, style_emb, #
                          source_pos_emb, future_motion_emb), axis=0) #[4,batch_size,dim]
    ...
    def interface(self, x, timesteps, y=None):
        ...
        source_pos = y['source_pos']

        return self.forward(x, timesteps, source_pos,  style_idx)
...
class MotionProcess(nn.Module):
    ...
    def forward(self, x):
        bs, nframes, njoints, nfeats = x.shape
        x = x.reshape(1,bs, njoints*nfeats) 
...
class OutputProcess(nn.Module):
    def __init__(self, input_feats, latent_dim, njoints, nfeats, nembeds):
        super().__init__()
        self.input_feats = input_feats
        self.latent_dim = latent_dim
        self.njoints = njoints
        self.nfeats = nfeats
        self.nembeds = nembeds
        self.poseLinear = nn.Linear(self.latent_dim, self.input_feats)
        self.poseFinal = nn.Linear(self.nembeds,1)

    def forward(self, output):
        nframes, bs, d = output.shape
        output = self.poseLinear(output)  #[nebs,ts,njoints*nfeats]
        output = output.permute(1,2,0)#[ts,njoints*nfeats,nebs]
        output = self.poseFinal(output) #[bs,njoints*nfeats,1]
        output = output.permute(0,2,1).reshape(bs,1, self.njoints, self.nfeats)
